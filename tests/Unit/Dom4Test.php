<?php
    ...
    final class Dom4Test extends TestCase
    {
            ...
            $registery[] = Field::ins('用户名', 'username', RawInput::class)->setDataMap([
                ...
                switch ($registery->getModel())
                {
                    case $registery::MODE_ADD :
                        $inputItem->setLabelStr($field->getTitle() . 'MODE_ADD');

                        break;
                    case $registery::MODE_EDIT :
                        $inputItem->setLabelStr($field->getTitle() . 'MODE_EDIT');
...
//                $colItem->setIsHidden(true);
                    ...
                    case $registery::MODE_BATCH_MODIFILY :
                        $inputItem->setLabelStr($field->getTitle() . 'MODE_BATCH_MODIFILY');

                        break;
                    case $registery::MODE_SEARCH :
                        $inputItem->setLabelStr($field->getTitle() . 'MODE_SEARCH');
...
//        ->setModel(FieldRegistry::MODE_ADD)
                ...
                ->renderAllFields(function(Form $this_, array &$_i, string $fieldsContents) {
                    $this_->btnXs();
                    $this_->withBorder();
