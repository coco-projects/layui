    ...
    var call = {
        ...
        add: function (e, tagElem, options) {
            var filter       = tagElem.attr("lay-filter"),
                ...
                newTag       = "<button lay-id=\"" + (options.id || "") + "\"" + (options.attr ? " lay-attr=\"" + options.attr + "\"" : "") + " type=\"button\" class=\"" + TAG_ITEM + "\"><span class='tag_text'>" + (options.text || "unnaming") + "</span></button>";
        ...
        input: function (e, othis) {
            ...
            inpatNewTag.children(".layui-input").on("blur", function () {
                ...
                if (event.keyCode == 13)
                {
                    layui.layer.msg("点击空白处或者按 Tab 键完成输入");
                    return false;
                }
            }).focus();
